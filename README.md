...
| 2 | Quant Engine — Indicator, Strategy, Backtest | MVP+ 완료 on `feat/analysis-quant` ([docs](docs/analysis-quant/README.md)); 확장은 같은 브랜치·[PLAN 로드맵](docs/analysis-quant/PLAN.md) |
...
투자 성향은 셀프 라벨 대신 Q&A로 도출합니다 (`scripts/assess_risk_profile.py`, Grable–Lytton + capacity). 프로필 JSON을 `--profile`로 시뮬에 넘기면 TradingPersona 가드와 sell grounding이 적용됩니다.

### 환경 (로컬 `uv run` / Docker 공통)

```bash
# .env — Mac/Windows 모두 QSEED_HOST_PATH만 본인 경로로 설정
QSEED_HOST_PATH=/absolute/path/to/Q-SEED/data   # Windows 예: D:/Users/User/Q-SEED/data
